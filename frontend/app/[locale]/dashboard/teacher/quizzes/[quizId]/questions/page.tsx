...
import { getQuizById, deleteQuestionFromQuiz } from "@/lib/api/quiz";
...
const QuizQuestionsPage = () => {
    ...
    const confirmDeleteQuestion = async () => {
        if (questionToDelete === null || !quiz?.questions) return;

        try {
            const success = await deleteQuestionFromQuiz(
                quizId,
                questionToDelete,
            );
            if (success) {
            ...
            <header className="mb-8 flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-semibold">
                        {quiz?.title}&apos;s Questions
                    </h1>
                    <p className="p-lead">Manage quiz questions</p>
                </div>
                <Button asChild className="btn btn-primary">
                    <Link
                        href={`/dashboard/teacher/quizzes/${quizId}/questions/new`}
                    >
                            ...
                            >
                                    ...
                                    <div className="flex items-start justify-between">
                                        ...
                                        <div className="ml-4 flex space-x-2">
                                            <Button
                                                ...
                                                onClick={() =>
                                                    router.push(
                                                        `/dashboard/teacher/quizzes/${quizId}/questions/edit/${index}`,
                                                    )
                                                }
                                            >
                                ...
                                <div className="bg-muted/20 text-muted-foreground border-t px-4 py-2 text-xs">
                                    {question.type === "multiple_choice"
                                        ? `Multiple Choice • ${question.answers?.length || 0} options`
                                        : "Essay Question"}
                                </div>
                            </div>
                        ),
