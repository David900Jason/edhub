...
const AssignmentQuestionsPage = () => {
    const router = useRouter();
    const t = useTranslations("TEACHER_DASHBOARD.ASSIGNMENTS.QUESTIONS");
    ...
    const confirmDeleteQuestion = async () => {
        if (questionToDelete === null || !assignment?.questions) return;
        if (questionToDelete === null || !assignment?.questions) return;

        try {
            const success = await deleteQuestionFromAssignment(
                assignmentId,
                questionToDelete,
            );
            if (success) {
    ...
    return (
        <section>
            <header className="mb-8 flex flex-col items-center justify-between max-sm:gap-6 sm:flex-row">
                <div>
                    <h1 className="text-3xl font-semibold max-sm:text-center">
                        {assignment?.title}
                        {t("title")}
                    </h1>
                    <p className="p-lead max-sm:text-center">
                        {t("description")}
                    </p>
                </div>
                <Button asChild className="btn btn-primary">
                    <Link
                        href={`/dashboard/teacher/assignments/${assignmentId}/questions/new`}
                    >
                                    ...
                                    <div className="flex items-start justify-between">
                                        ...
                                        <div className="ml-4 flex space-x-2">
                                            <Button
                                                ...
                                                onClick={() =>
                                                    router.push(
                                                        `/dashboard/teacher/assignments/${assignmentId}/questions/edit/${index}`,
                                                    )
                                                }
                                            >
            ...
            >
                <AlertDialogContent>
                    ...
                    <AlertDialogFooter>
                        <AlertDialogCancel>{t("cta3")}</AlertDialogCancel>
                        <AlertDialogAction
